...
function continuum_fit_spaxel(cube_fitter::CubeFitter, spaxel::CartesianIndex, λ::Vector{<:Real}, I::Vector{<:Real}, 
    σ::Vector{<:Real}, mask_lines::BitVector, N::Real, stellar_λref::Real, dust_λrefs::Vector{<:Real}; init::Bool=false, 
    ...
    if !bootstrap_iter
        ...
        pars_1 = vcat(p1_boots[1:(2+4cube_fitter.n_dust_cont+3)], p1_boots[end-1:end])
        pars_2 = p1_boots[(3+4cube_fitter.n_dust_cont+3):end-2]
    end
    ...
    function fit_step2(x, pfree, return_comps=false)
        ...
        ptot[lock_2] .= p2fix
        if !return_comps
            model_pah_residuals(x, ptot, cube_fitter.n_dust_feat)
        else
            model_pah_residuals(x, ptot, cube_fitter.n_dust_feat, true)
        end
    end
...
"""
    line_fit_spaxel(cube_fitter, spaxel, λ, I, σ, continuum, mask_lines, lsf_interp_func; init=init, use_ap=use_ap)
...
function line_fit_spaxel(cube_fitter::CubeFitter, spaxel::CartesianIndex, λ::Vector{<:Real}, I::Vector{<:Real},
    σ::Vector{<:Real}, continuum::Vector{<:Real}, lsf_interp_func::Function, N::Real; 
    ...
    if false
        ...
        fit_func = (x, p) -> -ln_likelihood(
                                ...
                                model_line_residuals(x, p, cube_fitter.n_lines, cube_fitter.n_comps, cube_fitter.lines, 
                                    cube_fitter.flexible_wavesol, lsf_interp_func), 
                                σnorm)
        x_tol = 1e-5
    ...
    I_model, comps = model_line_residuals(λ, popt, cube_fitter.n_lines, cube_fitter.n_comps, cube_fitter.lines, 
        cube_fitter.flexible_wavesol, lsf_interp_func, true)
...
function plot_spaxel_fit(λ::Vector{<:Real}, I::Vector{<:Real}, I_cont::Vector{<:Real}, σ::Vector{<:Real}, comps::Dict{String, Vector{T}}, 
    n_dust_cont::Integer, dust_types::Vector{Symbol}, n_dust_features::Integer, n_comps::Integer, line_wave::Vector{<:Real}, line_names::Vector{Symbol}, 
    screen::Bool, z::Real, χ2red::Real, name::String, label::String; backend::Symbol=:pyplot, I_boot_min::Union{Vector{<:Real},Nothing}=nothing, 
    ...
    if (backend == :plotly || backend == :both) && isnothing(range)
        ...
        ext_full = comps["abs_ice"] .* comps["abs_ch"]
        for comp ∈ keys(comps)
            if comp == "extinction"
        ...
        append!(traces, [PlotlyJS.scatter(x=λ, y=ext_full .* (
            (n_dust_cont > 0 ? sum([comps["dust_cont_$i"] for i ∈ 1:n_dust_cont], dims=1)[1] : zeros(length(λ))) .+ comps["stellar"]),
            mode="lines", line=Dict(:color => "green", :width => 2), name="Dust+Stellar Continuum")])
        # Individual continuum components
        append!(traces, [PlotlyJS.scatter(x=λ, y=ext_full .* comps["stellar"], mode="lines", line=Dict(:color => "magenta", :width => 1), name="Stellar continuum")])
        for i in 1:n_dust_cont
            append!(traces, [PlotlyJS.scatter(x=λ, y=ext_full .* comps["dust_cont_$i"], mode="lines", line=Dict(:color => "green", :width => 1), name="Dust continuum")])
        end
        # Summed up PAHs
        append!(traces, [PlotlyJS.scatter(x=λ, y=sum([comps["dust_feat_$i"] for i ∈ 1:n_dust_features], dims=1)[1],
    ...
    if backend == :pyplot || backend == :both
        ...
        # full PAH profile
        ax1.plot(λ, sum([comps["dust_feat_$i"] for i ∈ 1:n_dust_features], dims=1)[1] ./ norm ./ λ, "-", 
            color="#0065ff", label="PAHs")
        # full line profile
        ax1.plot(λ, sum([haskey(comps, "line_$(i)_$(j)") ? comps["line_$(i)_$(j)"] : zeros(length(λ)) 
            for i ∈ 1:length(line_wave), j ∈ 1:n_comps], dims=(1,2))[1] ./ norm ./ λ, "-", 
            color="rebeccapurple", label="Lines")
        # plot extinction
...
function _fit_spaxel_iterfunc(cube_fitter::CubeFitter, spaxel::CartesianIndex, λ::Vector{<:Real}, I::Vector{<:Real}, 
    ...
    popt_l, I_line, comps_line, n_free_l, perr_l = 
        @timeit timer_output "line_fit_spaxel" line_fit_spaxel(cube_fitter, spaxel, λ, I, σ, I_cont, 
    ...
    if !init
        p_dust, p_lines, p_dust_err, p_lines_err = 
            @timeit timer_output "calculate_extra_parameters" calculate_extra_parameters(λ, I, norm, cube_fitter.n_dust_cont,
                ...
                cube_fitter.flexible_wavesol, lsf_interp_func, popt_c[1:end-2], popt_l, perr_c[1:end-2], perr_l,
...
function fit_spaxel(cube_fitter::CubeFitter, cube_data::NamedTuple, spaxel::CartesianIndex; use_ap::Bool=false)
    ...
    if !isfile(joinpath("output_$(cube_fitter.name)", "spaxel_binaries", "spaxel_$(spaxel[1])_$(spaxel[2]).csv")) || cube_fitter.overwrite
        ...
        p_out, p_err = with_logger(spaxel_logger) do
            ...
            stellar_λref = clamp(Wein(cube_fitter.continuum.T_s.value), minimum(λ), maximum(λ))
            @debug "Reference wavelength for stellar continuum: $stellar_λref"
            dust_λrefs = [clamp(Wein(cube_fitter.continuum.T_dc[i].value), minimum(λ), maximum(λ)) for i in 1:cube_fitter.n_dust_cont]
            ...
            if cube_fitter.plot_spaxels != :none
                ...
                @timeit timer_output "plot_spaxel_fit" plot_spaxel_fit(λ, I, I_model, σ, comps, 
                    cube_fitter.n_dust_cont, cube_fitter.continuum.d_dc, cube_fitter.n_dust_feat, cube_fitter.n_comps, cube_fitter.lines.λ₀, cube_fitter.lines.names, 
...
function fit_stack!(cube_fitter::CubeFitter)
    ...
    # reference wavelengths for blackbody components
    stellar_λref = clamp(Wein(cube_fitter.continuum.T_s.value), minimum(λ_init), maximum(λ_init))
    @debug "Reference wavelength for stellar continuum: $stellar_λref"
    dust_λrefs = [clamp(Wein(cube_fitter.continuum.T_dc[i].value), minimum(λ_init), maximum(λ_init)) for i in 1:cube_fitter.n_dust_cont]
    @debug "Reference wavelengths for dust continua: $dust_λrefs"
